...
class Tokenizer extends _comments.default {
  ...
  readInt(radix, len, forceLen = false, allowNumSeparator = true) {
    const {
      n,
      pos
    } = (0, _helperStringParser.readInt)(this.input, this.state.pos, this.state.lineStart, this.state.curLine, radix, len, forceLen, allowNumSeparator, this.errorHandlers_readInt);
    this.state.pos = pos;
    return n;
  }
  ...
  checkKeywordEscapes() {
    ...
    if ((0, _types2.tokenIsKeyword)(type) && this.state.containsEsc) {
      this.raise(_parseError.Errors.InvalidEscapedReservedWord, {
        ...
        reservedWord: (0, _types2.tokenLabelName)(type)
      });
    }
  }
  ...
  unexpected(loc, type) {
    throw this.raise(_parseError.Errors.UnexpectedToken, {
      expected: type ? (0, _types2.tokenLabelName)(type) : null,
