...
class ExpressionParser extends _lval.default {
  ...
  parsePrivateName() {
    const node = this.startNode();
    const id = this.startNodeAt(this.state.start + 1, new _location.Position(this.state.curLine, this.state.start + 1 - this.state.lineStart, this.state.start + 1));
  ...
  parseTemplateElement(isTagged) {
    ...
    const elem = this.startNodeAt(elemStart, (0, _location.createPositionWithColumnOffset)(startLoc, 1));
  ...
  isAmbiguousAwait() {
    ...
    return type === 53 || type === 10 || type === 0 || (0, _types.tokenIsTemplate)(type) || type === 133 || type === 56 || this.hasPlugin("v8intrinsic") && type === 54;
  }
