      ...
      if (!process.env.BABEL_8_BREAKING) {
        if (type === 134) {
          ...
          tokens.splice(i, 1, new _tokenizer.Token({
            type: (0, _types2.getExportedToken)(27),
            value: "#",
            start: start,
            end: hashEndPos,
            startLoc: loc.start,
            ...
            type: (0, _types2.getExportedToken)(128),
            value: value,
            start: hashEndPos,
            end: end,
            startLoc: hashEndLoc,
        ...
        if ((0, _types2.tokenIsTemplate)(type)) {
          const {
            loc,
            start,
            value,
          ...
          if (input.charCodeAt(start) === 96) {
            startToken = new _tokenizer.Token({
              type: (0, _types2.getExportedToken)(22),
              value: "`",
              start: start,
              end: backquoteEnd,
              startLoc: loc.start,
            ...
            startToken = new _tokenizer.Token({
              type: (0, _types2.getExportedToken)(8),
              value: "}",
              start: start,
              end: backquoteEnd,
              startLoc: loc.start,
          ...
          if (type === 24) {
            ...
            endToken = new _tokenizer.Token({
              type: (0, _types2.getExportedToken)(23),
              value: "${",
              start: templateElementEnd,
              end: end,
              startLoc: templateElementEndLoc,
          ...
          tokens.splice(i, 1, startToken, new _tokenizer.Token({
            type: (0, _types2.getExportedToken)(20),
            value: templateValue,
            start: backquoteEnd,
...
class StatementParser extends _expression.default {
      ...
      case 68:
        ...
        if (context) {
          if (this.state.strict) {
            this.raise(_parseError.Errors.StrictFunction, {
              at: this.state.startLoc
            });
          } else if (context !== "if" && context !== "label") {
            this.raise(_parseError.Errors.SloppyFunction, {
              at: this.state.startLoc
            });
          }
        }
  ...
  parseCatchClauseParam() {
    ...
    this.checkLVal(param, {
      ...
      binding: _scopeflags.BIND_LEXICAL,
  ...
  parseLabeledStatement(node, maybeName, expr, context) {
    ...
    const kind = (0, _types2.tokenIsLoop)(this.state.type) ? "loop" : this.match(71) ? "switch" : null;
  ...
  registerFunctionStatementId(node) {
    if (!node.id) return;
    this.scope.declareName(node.id.name, this.state.strict || node.generator || node.async ? this.scope.treatFunctionsAsVar ? _scopeflags.BIND_VAR : _scopeflags.BIND_LEXICAL : _scopeflags.BIND_FUNCTION, node.id.loc.start);
  }
  ...
  isExportDefaultSpecifier() {
    ...
    if ((0, _types2.tokenIsIdentifier)(type)) {
      ...
      if ((type === 126 || type === 125) && !this.state.containsEsc) {
        ...
        if ((0, _types2.tokenIsIdentifier)(nextType) && nextType !== 97 || nextType === 5) {
    ...
    if (type === 26) {
      ...
      if (this.hasPlugin("decorators")) {
        if (this.getPluginOption("decorators", "decoratorsBeforeExport")) {
          throw this.raise(_parseError.Errors.DecoratorBeforeExport, {
            at: this.state.startLoc
          });
        }
